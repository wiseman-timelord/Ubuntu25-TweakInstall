...
- Note, Currently there is an issue with APT Center as a result of some of the tweaks, this will be investigated, but is an issue for now, so either, use the `Software Manager` installed by TweakInstall or dont use the tweaks/software manager (guessing).
